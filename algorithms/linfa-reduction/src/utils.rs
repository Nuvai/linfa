...
pub fn generate_swissroll(
    ...
    for i in 0..n_points {
        let z = rng.random_range(0.0..height);
        let phi: f64 = rng.random_range(0.0..10.0);
        //let offset: f64 = rng.random_range(-0.5..0.5);
        let offset = 0.0;
...
pub fn generate_convoluted_rings(
    ...
    for (n, (start, end)) in rings.iter().enumerate() {
        // inner circle
        for i in 0..n_points {
            let r: f64 = rng.random_range(*start..*end);
            let phi: f64 = rng.random_range(0.0..(f64::PI() * 2.0));
            let theta: f64 = rng.random_range(0.0..(f64::PI() * 2.0));
